...
app.post('/api/simulate', async (req, res) => {
  ...
  } else if (type === 'sensor_error') {
    const sensor = req.body.sensor || 'metal';
    mqttClient.publish('smartbin/sensor/status', JSON.stringify({ sensor, ok: false }));
  }

  res.json({ ok: true, message: 'Simulated message published to MQTT' });
});

// POST /api/reset-today — clears today's DB records and resets in-memory counters
app.post('/api/reset-today', async (req, res) => {
  ...
  try {
    ...
    await db.execute('DELETE FROM classification_events WHERE DATE(created_at) = ?', [today]);
    await db.execute('DELETE FROM fill_readings        WHERE DATE(created_at) = ?', [today]);
    Object.values(state.bins).forEach(b => { b.items_today = 0; b.last_item = null; b.fill_pct = 0; });
    console.log('[RESET] Today\'s data cleared via demo panel');
    res.json({ ok: true });
  } catch (e) {
